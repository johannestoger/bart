...
/* Primal Dual algorithm. Solves g(x) + f(Ax)
 * Assumes that g is in prox_ops[0], f is in prox_ops[1], A is in ops[1]
 */
void iter2_pridu(iter_conf* _conf,
		const struct operator_s* normaleq_op,
		unsigned int D,
		const struct operator_p_s* prox_ops[D],
		const struct linop_s* ops[D],
		...
		struct iter_monitor_s* monitor)
{
	...
	struct iter_pridu_conf* conf = CAST_DOWN(iter_pridu_conf, _conf);
	...
	pridu(conf->maxiter, eps * conf->tol, conf->tau, conf->sigma, conf->theta, conf->decay, 2 * md_calc_size(iv->N, iv->dims), 2 * md_calc_size(ov->N, ov->dims), select_vecops(image),
